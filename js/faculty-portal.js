...
function saveFacultyGradeEvaluation() {
    const studentId = document.getElementById('gradeStudentId').value.trim();
    const title = document.getElementById('gradeAssessmentTitle').value.trim();
    const marks = document.getElementById('gradeMarksObtained').value;
    const total = document.getElementById('gradeTotalMarks').value;
    const feedback = document.getElementById('gradeFeedback').value.trim();

    if (!studentId || !title || !marks || !total) {
        alert("Please provide Student ID, Assessment Title, Marks Obtained, and Total Marks.");
        return;
    }

    const trackerBody = document.getElementById('submissionTrackerBody');
    if (trackerBody) {
        const tr = document.createElement('tr');
        tr.style.borderBottom = "1px solid rgba(255,255,255,0.08)";
        tr.innerHTML = `
            <td style="padding: 10px; font-weight: bold; color: var(--accent-primary);">${studentId}</td>
            <td style="padding: 10px;">Student ${studentId}</td>
            <td style="padding: 10px;">${title}</td>
            <td style="padding: 10px;">Just now</td>
            <td style="padding: 10px;"><span style="background: rgba(34, 197, 94, 0.2); color: #4ade80; padding: 4px 10px; border-radius: 12px; font-size: 0.8rem; font-weight: bold;">Graded: ${marks}/${total}</span></td>
        `;
        trackerBody.insertBefore(tr, trackerBody.firstChild);
    }

    alert(`Grade Published Successfully!\nStudent: ${studentId}\nAssessment: ${title}\nScore: ${marks}/${total}`);

    document.getElementById('gradeStudentId').value = '';
    document.getElementById('gradeAssessmentTitle').value = '';
    document.getElementById('gradeMarksObtained').value = '';
    document.getElementById('gradeTotalMarks').value = '';
    document.getElementById('gradeFeedback').value = '';
}

function broadcastFacultyAnnouncement() {
    const title = document.getElementById('announceTitle').value.trim();
    const body = document.getElementById('announceBody').value.trim();

    if (!title || !body) {
        alert("Please fill in both Announcement Title and Content.");
        return;
    }

    const announceBoard = document.getElementById('facultyLiveAnnounceBoard');
    if (announceBoard) {
        ...
        item.innerHTML = `
            <strong style="color: var(--accent-primary); font-size: 0.95rem;">📢 ${title}</strong>
            <p style="margin: 4px 0 0; font-size: 0.88rem; color: var(--text-main);">${body}</p>
            <p style="margin: 4px 0 0; font-size: 0.8rem; color: var(--text-muted);">Posted just now by Faculty</p>
        `;
        announceBoard.insertBefore(item, announceBoard.firstChild);
    }

    alert("Announcement Broadcasted Successfully to Students!");
